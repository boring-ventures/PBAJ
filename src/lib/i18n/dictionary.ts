...
export function formatLocalizedDate(date: Date, locale: Locale, options?: Intl.DateTimeFormatOptions): string {
  ...
  return new Intl.DateTimeFormat(locale === 'es' ? 'es-ES' : 'en-US', formatOptions).format(date);
}
...
export const categoryTranslations = {
  ...
  program: {
    ...
    POLICY_DEVELOPMENT: { es: 'Desarrollo de Políticas', en: 'Policy Development' }
  },
  publication: {
...
export function getTranslatedStatus(
  ...
  const translations = statusTranslations[type] as Record<string, Record<string, string>>;
...
export function getTranslatedCategory(
  ...
  const translations = categoryTranslations[type] as Record<string, Record<string, string>>;
