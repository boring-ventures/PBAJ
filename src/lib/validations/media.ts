...
export const mediaBulkActionSchema = z.object({
  action: z.enum(['delete', 'move', 'tag', 'untag', 'public', 'private', 'optimize']),
...
export function getFileType(mimeType: string): keyof typeof MAX_FILE_SIZES | null {
  ...
  if (ALLOWED_AUDIO_TYPES.includes(mimeType as typeof ALLOWED_AUDIO_TYPES[number])) return 'audio';
  if (ALLOWED_DOCUMENT_TYPES.includes(mimeType as typeof ALLOWED_DOCUMENT_TYPES[number])) return 'document';
  if (ALLOWED_ARCHIVE_TYPES.includes(mimeType as typeof ALLOWED_ARCHIVE_TYPES[number])) return 'archive';
  return null;
}
